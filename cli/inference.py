...
def optimize_torch_settings(device):
    """Safe optimizations for PyTorch 1.12.1"""
    if device == "cuda":
        # Enable cuDNN benchmark for consistent input sizes
        torch.backends.cudnn.benchmark = True
        # Enable cuDNN deterministic for reproducible results (optional)
        # torch.backends.cudnn.deterministic = True

        # Set optimal number of threads
        torch.set_num_threads(min(8, os.cpu_count()))

        # Enable memory efficient attention (available in 1.12.1)
        os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "max_split_size_mb:128"
        torch.backends.cuda.matmul.allow_tf32 = True


def main(args):
    optimize_torch_settings(args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--driven_audio",
        default="./data/examples/driven_audio/bus_chinese.wav",
        help="path to driven audio",
    )
    parser.add_argument(
        "--source_image",
        default="./data/examples/source_image/full_body_1.png",
    ...
    parser.add_argument(
        "--checkpoint_dir", default="./data/checkpoints", help="path to output"
    )
    parser.add_argument("--result_dir", default="./data/results", help="path to output")
    parser.add_argument(
    ...
    parser.add_argument(
        "--bfm_folder", type=str, default="./data/checkpoints/BFM_Fitting/"
    )
    parser.add_argument(
